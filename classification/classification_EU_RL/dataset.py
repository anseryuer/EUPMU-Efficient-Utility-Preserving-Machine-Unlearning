...
class SVHN_(SVHN):
    ...
    def __init__(self, data_dir, split, transform, download):
        super(SVHN_, self).__init__(data_dir, split=split, transform=transform, download=download)
        self.temp=None
...
def svhn_dataloaders(
    ...
    train_set = SVHN_(data_dir, split="train", transform=train_transform, download=True)

    test_set = SVHN_(data_dir, split="test", transform=test_transform, download=True)
