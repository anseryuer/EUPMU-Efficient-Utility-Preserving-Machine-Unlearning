...
from mtl.single_side_pcgrad import SSPCGrad
...
class Diffusion(object):
    ...
    def train_forget(self):
        ...
        for step in range(0, config.training.n_iters):
            ...
            t = torch.cat([t, self.num_timesteps - t - 1], dim=0)[:n]
            fgt_loss = loss_registry_conditional[config.model.type](
                model, x_forget, t, c_forget, e_forget, b, cond_drop_prob=0.0
            )
            rmb_loss = loss_registry_conditional[config.model.type](
                model, x_remember, t, c_remember, e_remember, b, cond_drop_prob=0.0
            )

            pc_adam.zero_grad()
            pc_adam.pc_backward([fgt_loss,rmb_loss])
            pc_adam.step()
            #####################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ###????????????????????bushizhelibendan
            if (step + 1) % config.training.log_freq == 0:
                logging.info(
                    f"pcgrad working, step: {step}, forget loss: {fgt_loss.item()}, remember loss: {rmb_loss.item()}"
                )
    ...
    def saliency_unlearn(self):
        ...
        for step in range(0, self.config.training.n_iters):
            ...
            remain_loss = remain_loss * args.alpha
            pc_adam.zero_grad()
            pc_adam.pc_backward([forget_loss,remain_loss])
            pc_adam.step()

            ######################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if (step + 1) % self.config.training.log_freq == 0:
                end = time.time()
                logging.info(f"pcgrad2 working, step: {step}, forget loss: {forget_loss.item()}, remember loss: {remain_loss.item()}, time: {end-start}")
                start = time.time()
