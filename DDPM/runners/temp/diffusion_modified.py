...
class Diffusion(object):
    ...
    def saliency_unlearn(self):
        ...
        unlearning_method = "eu"
        assert unlearning_method in ["eu", "famo", "sspcgrad", "base"]
        if unlearning_method == "eu":
            weight_opt = EU(device="cuda:0")
            weight_opt.set_min_losses(torch.Tensor([0.0, 0.0]).to("cuda"))
        elif unlearning_method == "famo":
            weight_opt = FAMO(n_tasks=2,device="cuda:0")
            weight_opt.set_min_losses(torch.Tensor([0.0, 0.0]).to("cuda"))
        elif unlearning_method == "sspcgrad":
