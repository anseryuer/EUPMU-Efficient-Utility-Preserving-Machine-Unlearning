...
class FAMO:
    ...
    def __init__(
        ...
        temperature: float = 1.0, # the temperature of the softmax
    ):
        self.min_losses = torch.zeros(n_tasks).to(device)
        self.w = torch.tensor([-1.0,1.0], device=device, requires_grad=True)
    ...
    def get_weighted_loss(self, losses):
        ...
        z = F.softmax(self.w / self.temperature, -1)
    ...
    def update(self, curr_loss):
        ...
        with torch.enable_grad():
            d = torch.autograd.grad(F.softmax(self.w / self.temperature, -1),
                                    self.w,
                                    grad_outputs=delta.detach())[0]
        self.w_opt.zero_grad()
        #print(self.w.grad.size())
        self.w.grad = d
        self.w_opt.step()
    ...
    def backward(
        ...
        print(f"losses before weight update {losses.cpu().detach().numpy()}, weighted loss: {loss.item()}, weights: {F.softmax(self.w, -1).cpu().detach().numpy()}")
        loss.backward()
