...
class SSPCGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        assert num_task == 2,"for unlearning"
        g_i, g_j = pc_grad[0], pc_grad[1]
        g_i_g_j = torch.dot(g_i, g_j) - 0.0001
        if g_i_g_j > 0:
            #print("crop it!")
            g_j = (g_i_g_j) * g_j / (g_j.norm()**2)
        ...
        if self._reduction:
            merged_grad[shared] = torch.stack([g[shared]
                                           for g in [g_i, g_j]]).mean(dim=0)
        elif self._reduction == 'sum':
            merged_grad[shared] = torch.stack([g[shared]
                                           for g in [g_i, g_j]]).sum(dim=0)
...
if __name__ == '__main__':
    ...
    net = TestNet()
    y_pred = net(x)
    pc_adam = SSPCGrad(optim.Adam(net.parameters()))
    pc_adam.zero_grad()
    ...
    pc_adam.pc_backward_v2([loss1], [loss2])
    for p in net.parameters():
        print(p.grad)
