...
class CocoBase(Dataset):
    ...
    def __init__(
        ...
        assert data_json.split("/")[-1] in [
            f"captions_train{self.year()}.json",
        ...
        if self.use_segmenation:
            if self.stuffthing:
                self.segmentation_prefix = (
                    f"data/cocostuffthings/val{self.year()}"
                    if data_json.endswith(f"captions_val{self.year()}.json")
                    else f"data/cocostuffthings/train{self.year()}"
                )
            else:
                self.segmentation_prefix = (
                    f"data/coco/annotations/stuff_val{self.year()}_pixelmaps"
                    if data_json.endswith(f"captions_val{self.year()}.json")
                    else f"data/coco/annotations/stuff_train{self.year()}_pixelmaps"
                )
        ...
        if force_no_crop:
            ...
            self.preprocessor = albumentations.Compose(
                [self.rescaler], additional_targets={"segmentation": "image"}
            )
...
if __name__ == "__main__":
    with open(
        "data/coco/annotations2014/annotations/captions_val2014.json", "r"
    ) as json_file:
